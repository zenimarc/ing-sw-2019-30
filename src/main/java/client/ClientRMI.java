...
public class ClientRMI extends UnicastRemoteObject implements Client, Observer {
    ...
    private void storeToken(UUID token){
        File file = new File("data"+File.separator+"tokens"+this.nickname+"-token.txt");
    ...
    private boolean loadTokenFromFile(){
        try (BufferedReader buff = new BufferedReader(new FileReader("data"+File.separator+"tokens"+this.nickname+"-token.txt"))){
